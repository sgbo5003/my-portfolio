...
const Toolbar = () => {
  ...
  useEffect(() => {
    updateDOM(config);
    document.body.setAttribute('data-app-toolbar-enabled', 'true');
  }, [config]);
...
const updateDOM = (config: ILayout) => {
  ...
  if (appToolbarStickyEnabled) {
    ...
    if (config.app?.toolbar?.fixed?.mobile) {
      document.body.setAttribute('data-app-toolbar-fixed-mobile', 'true');
    }

    setTimeout(() => {
      const toolbarElement = document.getElementById('app_toolbar');
      // toolbar
      if (toolbarElement) {

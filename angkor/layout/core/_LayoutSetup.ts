...
const getLayoutFromSessionStorage = (): ILayout => {
  if (typeof window !== 'undefined') {
    const ls = sessionStorage.getItem(LAYOUT_CONFIG_KEY);
    if (ls) {
      try {
...
const setLayoutIntoSessionStorage = (config: ILayout) => {
  try {
    sessionStorage.setItem(LAYOUT_CONFIG_KEY, JSON.stringify(config));
  } catch (er) {
    console.error(er);
  }
};
...
class LayoutSetup {
  ...
  public static config: ILayout = getLayoutFromSessionStorage();
  ...
  private static initLayoutSettings(config: ILayout): ILayout {
    ...
    if (typeof window === 'object') {
      ...
      document.body.setAttribute('data-name', 'um');
  ...
  public static setConfig(config: ILayout): void {
    setLayoutIntoSessionStorage(config);
  }
...
export {
  LayoutSetup,
  getLayoutFromSessionStorage,
  setLayoutIntoSessionStorage,
  getEmptyCssClasses,

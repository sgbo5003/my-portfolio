...
class StepperComponent {
  ...
  private _goTo = (index: number) => {
    EventHandlerUtil.trigger(this.element, 'ak.stepper.change');
    ...
    EventHandlerUtil.trigger(this.element, 'ak.stepper.changed');
  };
  ...
  private initHandlers = () => {
    this.btnNext?.addEventListener('click', (e: Event) => {
      ...
      EventHandlerUtil.trigger(this.element, 'ak.stepper.next', e);
    });

    this.btnPrev?.addEventListener('click', (e: Event) => {
      e.preventDefault();

      EventHandlerUtil.trigger(this.element, 'ak.stepper.previous', e);
    });

    DOMEventHandlerUtil.on(this.element, '[data-stepper-action="step"]', 'click', (e: Event) => {
      e.preventDefault();
  ...
  private refreshUI = () => {
    ...
    for (var i = 0, len = elements.length; i < len; i++) {
      ...
      if (index === this.currentStepIndex) {
        ...
        if (this.options.animation !== false && element.getAttribute('data-stepper-element') === 'content') {
  ...
  public static bootstrap(attr: string = '[data-stepper]') {

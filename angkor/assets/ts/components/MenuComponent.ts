...
class MenuComponent {
  ...
  private _setTriggerElement = () => {
    ...
    if (target) {
      ...
      this.triggerElement = this.element.closest('[data-menu-trigger]') as HTMLElement;
    } else if (
      this.element.parentNode &&
      getElementChild(this.element.parentNode as HTMLElement, '[data-menu-trigger]')
    ) {
      const child = getElementChild(this.element.parentNode as HTMLElement, '[data-menu-trigger]');
      if (child) {
  ...
  private _getItemElement = (_element: HTMLElement): HTMLElement | undefined => {
    ...
    const item = _element.closest<HTMLElement>('.menu-item[data-menu-trigger]');
    if (item) {
      return item;
    }
  ...
  private _getItemParentElement = (item: HTMLElement) => {
    ...
    const parentItem = sub.closest<HTMLElement>('.menu-item[data-menu-trigger]');
    if (sub && parentItem) {
      return parentItem;
    }
  ...
  private _getItemChildElement = (item: HTMLElement): HTMLElement | null => {
    ...
    if (selector) {
      ...
      const element = selector.querySelector<HTMLElement>('.menu-item[data-menu-trigger]');
      if (element) {
        return element;
      }
    }
  ...
  private _isItemSubShown = (item: HTMLElement) => {
    ...
    if (sub) {
      if (this._getItemSubType(item) === 'dropdown') {
        ...
        // console.log(subHTMLElement);
        // console.log(subHTMLElement.classList.contains('show'));
        // console.log(subHTMLElement.hasAttribute('data-popper-placement'));
  ...
  private _hasItemSub = (item: HTMLElement) =>
    item.classList.contains('menu-item') && item.hasAttribute('data-menu-trigger');
  ...
  private _showDropdown = (item: HTMLElement) => {
    ...
    // console.log(item);
    if (EventHandlerUtil.trigger(this.element, 'ak.menu.dropdown.show') === false) {
      return;
    }
    ...
    EventHandlerUtil.trigger(this.element, 'ak.menu.dropdown.shown');
  };

  // Init dropdown popper(new)
  private initDropdownPopper = (item: HTMLElement, sub: HTMLElement) => {
    // console.log('initDropdownPopper');
  ...
  private _hideDropdown = (item: HTMLElement) => {
    ...
    if (EventHandlerUtil.trigger(this.element, 'ak.menu.dropdown.hide') === false) {
      return;
    }
    // console.log('_hideDropdown : 1');
    const sub = this._getItemSubElement(item);
    if (sub) {
    ...
    // console.log('_hideDropdown : 3');
    // Append the sub back to it's parent
    if (this._getItemOption(item, 'overflow') === true) {
      // console.log('_hideDropdown : 4');
      ...
      if (sub) {
        // console.log('_hideDropdown : 5');
        DataUtil.remove(item, 'sub');
        DataUtil.remove(sub, 'item');
        DataUtil.remove(sub, 'menu');
      }
    }
    // console.log('_hideDropdown : 6');
    if (DataUtil.has(item, 'popper') === true) {
      // console.log('_hideDropdown : 7');
      // @ts-ignore
      DataUtil.get(item, 'popper').destroy();
      DataUtil.remove(item, 'popper');
    }
    // console.log('_hideDropdown : 8');
    // Destroy popper(new)
    this.destroyDropdownPopper(item);
    // console.log('_hideDropdown : 9');
    EventHandlerUtil.trigger(this.element, 'ak.menu.dropdown.hidden');
    // console.log('_hideDropdown : 10');
  };
  ...
  private _showAccordion = (item: HTMLElement) => {
    // console.log('_showAccordion');
    if (EventHandlerUtil.trigger(this.element, 'ak.menu.accordion.show') === false) {
      return;
    }
    ...
    if (subElement) {
      ...
      slideDown(sub, this.options.accordion.slideSpeed, () => {
        ...
        item.classList.add('show');
        sub.classList.add('show');
        EventHandlerUtil.trigger(this.element, 'ak.menu.accordion.shown');
      });
    }
  };

  private _hideAccordion = (item: HTMLElement) => {
    if (EventHandlerUtil.trigger(this.element, 'ak.menu.accordion.hide') === false) {
      return;
    }
    ...
    if (sub) {
      slideUp(sub, this.options.accordion.slideSpeed, () => {
        ...
        EventHandlerUtil.trigger(this.element, 'ak.menu.accordion.hidden');
      });
    }
  };
  ...
  private _toggle = (item: HTMLElement) => {
    ...
    if (this._isItemSubShown(item) === true) {
      // console.log('_toggle : true');
      this._hide(item);
    } else {
      // console.log('_toggle : false');
      this._show(item);
    }
  };
  ...
  private _link = (element: HTMLElement, e: Event) => {
    if (EventHandlerUtil.trigger(this.element, 'ak.menu.link.click') === false) {
      return;
    }

    // Dismiss all shown dropdowns
    MenuComponent.hideDropdowns(undefined);
    EventHandlerUtil.trigger(this.element, 'ak.menu.link.clicked');
  };
  ...
  public off = (name: string, handlerId: string) => EventHandlerUtil.off(this.element, name, handlerId);

  // public static methods
  // Get AKMenu instance by element
  ...
  public static hideDropdowns = (skip: HTMLElement | undefined) => {
    ...
    const items = document.querySelectorAll<HTMLElement>('.show.menu-dropdown[data-menu-trigger]');

...
class ImageInputComponent {
  ...
  private _change = (e: Event) => {
    ...
    if (this.inputElement !== null && this.inputElement.files && this.inputElement.files[0]) {
      // Fire change event
      if (EventHandlerUtil.trigger(this.element, 'ak.imageinput.change', e) === false) {
        return;
      }
      ...
      // Fire removed event
      EventHandlerUtil.trigger(this.element, 'ak.imageinput.changed', e);
    }
  };
  ...
  private _cancel = (e: Event) => {
    ...
    // Fire cancel event
    if (EventHandlerUtil.trigger(this.element, 'ak.imageinput.cancel', e) === false) {
      return;
    }
    ...
    // Fire canceled event
    EventHandlerUtil.trigger(this.element, 'ak.imageinput.canceled', e);
  };
  ...
  private _remove = (e: Event) => {
    ...
    // Fire remove event
    if (EventHandlerUtil.trigger(this.element, 'ak.imageinput.remove', e) === false) {
      return;
    }
    ...
    // Fire removed event
    EventHandlerUtil.trigger(this.element, 'ak.imageinput.removed', e);
  };

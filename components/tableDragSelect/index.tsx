...
export default class TableDragSelect extends React.Component<any, any> {
  static propTypes = {
    value: (props: any) => {
    ...
    children: (props: any) => {
      ...
      const trs: any = React.Children.toArray(props.children);
  ...
  handleTouchMoveCell = (e: any) => {
    if (this.state.selectionStarted) {
      ...
      if (endRow !== row || endColumn !== column) {
        const nextRowCount = startRow === null && endRow === null ? 0 : Math.abs(row - (startRow ?? 0)) + 1;
        const nextColumnCount =
          startColumn === null && endColumn === null ? 0 : Math.abs(column - (startColumn ?? 0)) + 1;
  ...
  handleTouchEndWindow = (e: any) => {
    ...
    if (this.state.selectionStarted && (isLeftClick || isTouch)) {
      ...
      const minRow = Math.min(Number(this.state.startRow), Number(this.state.endRow));
      const maxRow = Math.max(Number(this.state.startRow), Number(this.state.endRow));
      for (let row = minRow; row <= maxRow; row++) {
        const minColumn = Math.min(Number(this.state.startColumn), Number(this.state.endColumn));
        const maxColumn = Math.max(Number(this.state.startColumn), Number(this.state.endColumn));
  ...
  isCellBeingSelected = (row: any, column: any) => {
    const minRow = Math.min(Number(this.state.startRow), Number(this.state.endRow));
    const maxRow = Math.max(Number(this.state.startRow), Number(this.state.endRow));
    const minColumn = Math.min(Number(this.state.startColumn), Number(this.state.endColumn));
    const maxColumn = Math.max(Number(this.state.startColumn), Number(this.state.endColumn));
...
class Cell extends React.Component<any, any> {
  ...
  shouldComponentUpdate = (nextProps: any) =>
    this.props.beingSelected !== nextProps.beingSelected || this.props.selected !== nextProps.selected;
  ...
  render = () => {
    let { className = '', disabled, beingSelected, selected, onTouchStart, onTouchMove, ...props } = this.props;
    if (disabled) {
